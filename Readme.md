...
Далее нужно правильно настроить размер хеш-таблицы. Установим размер хеш-таблицы 15013.
...
Чтобы правильно найти узкие места программы, нужно уменьшить вклад во время выполнения программы функций, которые выполняются один раз при инициализации таблицы. Для этого будем вызывать оптимизируемую функцию ~10^8 раз. Тогда профилировщик правильно покажет какая функция самая нагруженная.
...
Изменение типа данных ускорило поиск в 1,4 раза и вставку-удаление в 1,5 раза по сравнению с предыдущей версией.

Найдем общий прирост производительности. Поиск ускорился в 1,8 раза, а вставка-удаление в 1,7 раза.
...
В результатах профилирования вставки-удаления элементов видно, что удаление элемента из таблицы `HashTableRemove` и добавление элемента в таблицу `HashTableInsert` занимают 5% и 4% соответственно от общего времени выполнения. Однако оптимизировать их не получится, так как при попытке переписать их на ассемблере или добавить ассемблерную вставку, читаемость кода ухудшится. Частные случаи применить к этим функциям тоже не получится.
...
Время выполнения 18,2 с

<p align="center">
    <img src="Profiling/find asm optimization release.png"  width="600"/>
</p>

#### Вставка-удаление элемента

Время выполнения 32,1 с

<p align="center">
    <img src="Profiling/insert-remove asm optimization release.png"  width="600"/>
</p>
...
|:---------:|:---:|:--------------:|
|Оптимизация хеш-функции `SSE`-инструкцией|13,4|22,4|
|Ассемблерная оптимизация|18,2|32,1|
...
Время выполнения 15,7 с

<p align="center">
    <img src="Profiling/find hash optimization release x86.png"  width="600"/>
</p>

#### Вставка-удаление элемента

Время выполнения 26,7 с

<p align="center">
    <img src="Profiling/insert-remove hash optimization release x86.png"  width="600"/>
</p>
...
Время выполнения 15,8 с

<p align="center">
    <img src="Profiling/find inline optimization release x86.png"  width="600"/>
</p>

#### Вставка-удаление элемента

Время выполнения 26,9 с

<p align="center">
    <img src="Profiling/insert-remove inline optimization release x86.png"  width="600"/>
</p>
...
|Оптимизация|Поиск|Вставка-удаление|
|:---------:|:---:|:--------------:|
|Оптимизация хеш-функции `SSE`-инструкцией|13,6|26,7|
|Inline оптимизация|15,8|26,9|
...
Среднее время выполнения 23,1 с
...
Среднее время выполнения 40,0 с
...
Среднее время выполнения 10,2 с

<p align="center">
    <img src="Profiling/find hash optimization releaseMax.png" width="600"/>
</p>

#### Вставка-удаление элемента

Среднее время выполнения 20,9 с

<p align="center">
    <img src="Profiling/insert-remove hash optimization releaseMax.png"  width="600"/>
</p>
...
|Оптимизация|Поиск|Вставка-удаление|
|-------------------------:|:-----:|:-----:|
|Не оптимизированная программа, `CompReleaseProfiling`|24,3|39|
|Не оптимизированная программа, `CompReleaseMax`|23,1|40,0|
|`SSE` Оптимизация хеш-функции, `CompReleaseProfiling`|13,4|22,4|
|`SSE` Оптимизация хеш-функции, `CompReleaseMax`|10,2|20,9|

Сравнивая между собой опции компиляции `CompReleaseProfiling`, поиск ускорился в 1,8 раза, вставка-удаление ускорилась в 1,7  раза.

Сравнивая между собой опции компиляции `CompReleaseMax`, поиск ускорился в 2,3 раз, вставка-удаление ускорилась в 1,9 раза.

### Оптимизация программы. Вывод

Применяя аппаратные оптимизации удалось добиться ускорения поиска в 2,3 раз, вставка-удаление в 1,9 раза.
